            ...
            <tr v-for="(item, i) in items" :key="item.id">
              ...
              <td>
                <template v-if="item.edit">
                  <v-btn class="me-2" icon="mdi-check" @click="onEditSubmit(item.id, i)"></v-btn>
                  <v-btn icon="mdi-undo" @click="cancelEditItem(item.id)"></v-btn>
                </template>
                <template v-else>
                  <v-btn class="me-2" icon="mdi-pencil" @click="editItem(item.id)"></v-btn>
...
const { addItem, editItem, cancelEditItem, confirmEditItem, delItem, delFinishedItem } = list
